...
#echo "cat $unused_images | sed -e 's/\(.*\)/\"\1\" /' | tr -d \"\n\" > $unused_images_flatten"
...
for library in $repository_dir/*; do
	...
	#echo "jq '.' \"$repo/_index_images\" > \"$output_dir/$(basename $repo)/_index_images.old\""
	jq '.' "$repo/_index_images" > "$output_dir/$(basename $repo)/_index_images.old"
	#echo "jq -s '.[0] - [ .[1:][] | {id: .} ]' \"$repo/_index_images\" > \"$output_dir/$(basename $repo)/_index_images\""
        jq -s '.[0] - [ .[1:][] | {id: .} ]' "$repo/_index_images" > "$output_dir/$(basename $repo)/_index_images"
